    ...
    if isinstance(clf, (RandomForestClassifier, DecisionTreeClassifier)):
        if clf.n_outputs_ == 1:
            ...
            if proba.ndim == 1:  # Handle edge case for 1D array
                ...
                result = proba[:, 1] if proba.shape[1] > 1 else proba[:, 0]  # Return first index if only one class exists
        else:  # Multi-output classification
            result = np.array(clf.predict_proba(X))[:, :, 1].T
        return squeeze_output(result)

    elif isinstance(clf, (RandomForestRegressor, DecisionTreeRegressor, RandomSurvivalForest, SurvivalTree)):
        ...
        raise ValueError(f"Tree learner '{clf.__class__.__name__}' not recognized or not implemented")
