...
for folder in testing_dnames:
    ...
    while j < 5:  # old: folder.split(".csv")[0]
        ...
        if SETUP.lower() in survival_key_list:
            rf = RandomSurvivalForest(
                n_estimators=100, min_samples_split=10, n_jobs=JOBS, random_state=0
            )

        elif SETUP.lower() in binary_key_list + multi_label_key_list:
            rf = RandomForestClassifier(
                n_estimators=100, min_samples_split=5, n_jobs=JOBS, random_state=0
            )
    ...
    results_df = pd.concat(
        [results_df, performances], axis=0
